...
for q, p in enumerate(ps):
	...
	for s, sigma in enumerate(sigmas):
		...
		if(sigma==1/4):
			#Ts = 1-np.array([np.sum(p_k*(1/(1+(np.sqrt(1-4*((sigma*gamma-sigma*b)/(sigma+gamma)**2)))/(k**2)))) for b in betas])
			Ts = 1-np.array([np.sum(p_k*(1/(1+((b*tau_SEIR)/(k*(k)))))) for b in betas])
			u_sols = np.array([u[np.array([np.sum(p_k*k*(1+(i-1)*j)**(k-1)) for i in u])>(np.sum(p_k*k)*u)][-1] for j in Ts])
			S = 1 - np.array([np.sum(p_k*(1-j+(i*j))**(2*k)) for (i, j) in zip(u_sols, Ts)])

		fig2, ax2 = plt.subplots(figsize = (10,8), gridspec_kw={'bottom': 0.13,'left':.2})
		...
		for b, beta in enumerate(betas):
			...
			data_stats = np.loadtxt('../../../../Dropbox/Research/Epidemiology_2020/Text_files/Stochastic/Networks/barabasi-albert/k_normalization/stats_R0%.1f_sigma%.1f_N%d_p%.1f_barabasi-albert.txt'%(beta/gamma, sigma, N, p))
			...
			data_succ = np.histogram(np.concatenate((succ_nodes,[])), bins=np.logspace(np.log10(1), np.log10(np.max(data_stats[:,0])), 12), density = False)
			...
			#----METHOD 2----
			# degrees_epi, counts_epi = np.unique(succ_nodes, return_counts=True)
			# degrees_ext, counts_ext = np.unique(ext_nodes, return_counts=True)
			...
			# Log_Likelihood_data2 = np.log10(prob_epi_k0_data/p_epi)
			# #degrees_array = np.linspace(np.min(degrees_data), np.max(degrees_data), 200)

			#ax.plot(degrees/meanDegree, Log_Likelihood_data2, linestyle = '', marker = '*', color=colors_R[b], ms = 10)

			ax.plot(degrees_data/meanDegree, Log_Likelihood_data,linestyle = '', marker = '*', color=colors_R[b], ms = 14, label = r'%.1f'%(Ts[b]/T_c))
