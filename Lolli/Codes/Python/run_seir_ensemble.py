...
Q_days = np.array([0, 4, 8, 12, 14, 18])
...
for i, d in enumerate(Q_days):
	...
	for n_ensemble in tqdm(np.arange(N_ensemble)):
		model = ExtSEIRSNetworkModel(G=G_normal, beta=beta, sigma=sigma, lamda = lamda, gamma = gamma, gamma_asym=gamma, a = 1,  p=1,
		                          G_Q=G_normal, initI_asym=1, isolation_time = d, sigma_Q = 1000, lamda_Q = 1/4, gamma_Q_asym = 1/6,
		                          transition_mode = 'time_in_state', prevalence_ext = 0, eta = 1, mu_H = 1, xi = 1e-10, nu = 1e-10)
...
my_plot_layout(ax=ax1, xlabel = 'Infected')
fig1.savefig('../../Figures/1_Extended_Model/histograms.pdf')

ax2.plot(Q_days, I_avg_d)
my_plot_layout(ax=ax2, xlabel = 'Days', ylabel = 'Infected')
